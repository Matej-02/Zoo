...
function assignKeeper(employee) {
    ...
    const sick = state.animals.filter(animal => animal.sick).sort((a, b) => a.health - b.health)[0];
    if (sick) {
    ...
    const criticalHygiene = state.animals.filter(animal => animal.hygiene < 42).sort((a, b) => a.hygiene - b.hygiene)[0];
    if (criticalHygiene) {
    ...
    const criticalGrooming = state.animals.filter(animal => animal.grooming < 38).sort((a, b) => a.grooming - b.grooming)[0];
    if (criticalGrooming) {
    ...
    const dirtyPens = penCache.filter(pen => pen.animals.length && gateWorkspot(pen)).map(pen => ({ pen, waste: state.animalWaste.filter(item => pen.cellSet.has(key(item.x, item.y))).reduce((sum, item) => sum + item.amount, 0) })).filter(item => item.waste >= 3).sort((a, b) => b.waste - a.waste);
    ...
    const hygiene = state.animals.filter(animal => animal.hygiene < 68).sort((a, b) => a.hygiene - b.hygiene)[0];
    if (hygiene) {
    ...
    const grooming = state.animals.filter(animal => animal.grooming < 64).sort((a, b) => a.grooming - b.grooming)[0];
    if (grooming) {
    ...
    let target = null, lowest = 101;
    for (const pen of penCache) {
        if (!pen.animals.length || !gateWorkspot(pen)) continue;
        const hunger = pen.animals.reduce((sum, animal) => sum + animal.hunger, 0) / pen.animals.length;
        if (hunger < lowest) { lowest = hunger; target = pen; }
    }
    if (target && lowest < 72 && sendKeeperToPen(employee, target, { type: 'feed', penId: target.id }, `Walking to feed habitat ${target.id + 1}`)) return;
    const random = randomAccessiblePath(); employee.path = pathfind(employee.x, employee.y, random[0], random[1]); employee.task = { type: 'patrol' }; employee.status = 'Inspecting animal welfare';
}
...
function assignJanitor(employee) {
    if (state.litter.length) {
        ...
        for (const litter of state.litter) {
            const distance = Math.abs(litter.x - employee.x) + Math.abs(litter.y - employee.y);
            if (distance < bestDistance && accessiblePaths.has(key(litter.x, litter.y))) { best = litter; bestDistance = distance; }
        }
        if (best) {
            ...
            if (path.length || (Math.floor(employee.x) === best.x && Math.floor(employee.y) === best.y)) {
                employee.task = { type: 'clean', x: best.x, y: best.y }; employee.path = path; employee.status = 'Walking to public litter'; return;
            }
        }
    }
    const random = randomAccessiblePath(); employee.path = pathfind(employee.x, employee.y, random[0], random[1]); employee.task = { type: 'patrol' }; employee.status = 'Patrolling public paths';
}
