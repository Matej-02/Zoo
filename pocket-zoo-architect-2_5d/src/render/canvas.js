...
function drawAnimal(a) {
    ...
    const scale = ['elephant', 'hippo', 'polarBear', 'gorilla'].includes(id) ? 1.34 : ['giraffe', 'dolphin', 'crocodile'].includes(id) ? 1.26 : 1.18;
    ctx.save();
    ctx.translate(x, y + b);
    ctx.scale(scale, scale);
...
function drawPerson(person, type) {
    ...
    if (staffMember) {
        ...
        if (person.role === 'guide') {
            ...
            for (const q of [8, 11, 14])
                ellipse(q, -16, 1.5, 1.5, '#73a94b');
        }
