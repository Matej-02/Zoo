...
## Illustrated animal and mobile update

Every animal species now has a dedicated illustrated sprite asset in `assets/animals/`. The otter also includes a small multi-frame animation test. On touch devices, use one finger for construction and two fingers to pinch-zoom or move the camera. Mouse-wheel and button zoom preserve the point at the centre of the visible world.


## Painterly grass tile

The grass terrain now uses the supplied hand-painted isometric PNG asset from `assets/terrain/grass.png`. Other terrain types remain temporary until their replacement assets are supplied.
