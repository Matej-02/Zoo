...
function analyzePens() {
    ...
    for (const p of penCache) {
        const guestLitter = state.litter.filter(l => p.cellSet.has(key(l.x, l.y))).reduce((sum, item) => sum + item.amount * 4, 0);
        const animalWaste = state.animalWaste.filter(w => p.cellSet.has(key(w.x, w.y))).reduce((sum, item) => sum + item.amount * 4, 0);
        p.waste = state.animalWaste.filter(w => p.cellSet.has(key(w.x, w.y))).reduce((sum, item) => sum + item.amount, 0);
        p.cleanliness = clamp(100 - guestLitter - animalWaste, 0, 100);
    }
...
function animalNeeds(a, p) {
    ...
    if (a.hygiene < 55) {
        ...
        score -= Math.min(18, (55 - a.hygiene) * .35);
    }
    if (a.grooming < 55) {
        issues.push('needs grooming');
        score -= Math.min(15, (55 - a.grooming) * .3);
    }
    if (a.sick) {
        issues.push('is ill and needs treatment');
        score -= 28;
    }
    if (a.health < 65) {
        issues.push(`health is low (${Math.round(a.health)}%)`);
        score -= Math.min(22, (65 - a.health) * .45);
    }
    if (p.cleanliness < 55) {
