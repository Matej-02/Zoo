...
function moveEntity(e, dt, speed) {
    ...
    const [tx, ty] = e.path[0], gx = tx + .5, gy = ty + .5;
    const dx = gx - e.x, dy = gy - e.y, len = Math.hypot(dx, dy);
    const cx = clamp(Math.floor(e.x), 0, COLS - 1), cy = clamp(Math.floor(e.y), 0, ROWS - 1);
    const pathBoost = GROUND[tile(cx, cy).ground]?.walkSpeed || 1;
    const step = dt * speed * pathBoost;
    if (len <= Math.max(.05, step)) {
