...
function place(x, y) {
    ...
    if (d.animal) {
        ...
        state.animals.push({ id: uid(), name, species: tool, px: x + .5, py: y + .5, targetX: x, targetY: y, dir: 1, moveTimer: 0, hunger: 88, happiness: 70, hygiene: 90, grooming: 90, health: 100, sick: false, illnessDays: 0, ageDays: 365, juvenile: false, sex: Math.random() < .5 ? 'female' : 'male', penId: p.id, issues: [], animOffset: Math.random() * 10 });
        structureDirty = true;
        log(`${name} the ${d.name.toLowerCase()} arrived.`);
        pushZooEvent('animal', 'New animal arrived', `${name} the ${d.name.toLowerCase()} joined the zoo.`, { key: `arrival:${name}:${state.calendar.year}-${state.calendar.month}-${state.calendar.day}` });
        checkGoals();
        return;
    }
