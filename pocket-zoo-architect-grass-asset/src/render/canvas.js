...
function drawGround(t, x, y, locked = false) {
    ...
    if (!locked && isWaterGround(t.ground)) {
        ...
        gradient.addColorStop(0, t.ground === 'saltWater' ? '#347da6' : '#4b98bc');
        gradient.addColorStop(.45, definition.color);
        gradient.addColorStop(1, t.ground === 'saltWater' ? '#245f85' : '#2d789d');
        fill = gradient;
    }

    polygon(points, fill, locked ? 'rgba(29,58,42,.18)' : 'rgba(33,65,47,.12)', 1);

    const terrainSprite = !locked ? getTerrainSprite(t.ground) : null;
    if (terrainSprite) {
        const spriteWidth = ISO_TILE_W + 4;
        const spriteHeight = spriteWidth * terrainSprite.naturalHeight / terrainSprite.naturalWidth;
        ctx.drawImage(terrainSprite, center.x - spriteWidth / 2, points[0].y - 1, spriteWidth, spriteHeight);
    }
    ...
    if (locked) {
    ...
    else if (t.ground === 'grass' && !terrainSprite) {
        ...
        ctx.globalAlpha = .18;
        ctx.fillStyle = t.ground === 'saltWater' ? 'rgba(126,214,230,.24)' : 'rgba(153,228,236,.28)';
        for (let i = 0; i < 2; i++) {
            const offset = Math.sin(now + x * .6 + y * .35 + i) * 5;
            ctx.beginPath();
            ctx.ellipse(center.x - 10 + i * 16 + offset, center.y - 1 + i * 2, 11, 4, 0, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.globalAlpha = .5;
        ctx.strokeStyle = 'rgba(214,248,250,.46)';
        ctx.lineWidth = 1.1;
        for (let i = 0; i < 3; i++) {
            const offset = Math.sin(now + x * .7 + y * .4 + i * .75) * 3;
            ctx.beginPath();
            ctx.ellipse(center.x - 18 + i * 18 + offset, center.y - 2 + i * 2, 8.5, 2.2, 0, 0, Math.PI * 2);
            ctx.stroke();
        }
    }
...
function otterAnimationState(animal) {
    const dx = (animal.targetX ?? Math.floor(animal.px)) + .5 - animal.px;
    const dy = (animal.targetY ?? Math.floor(animal.py)) + .5 - animal.py;
    const moving = Math.hypot(dx, dy) > .12;
    const beat = performance.now() / 1000 + animal.animOffset * .8;
    if (!moving) {
        if (animal.sick || animal.health < 48 || Math.sin(beat * .55) < -0.9)
            return { sequence: 'sleep', frame: 0, bob: 0 };
        if (animal.hunger < 62 && Math.sin(beat * 1.2) > 0.72)
            return { sequence: 'eat', frame: 0, bob: .2 };
        return { sequence: 'idle', frame: Math.floor((performance.now() / 700 + animal.animOffset * 3) % 2), bob: .25 };
    }
    return { sequence: 'walk', frame: Math.floor((performance.now() / 180 + animal.animOffset * 5) % 3), bob: .55 };
}

function drawOtterSprite(animal) {
    const animation = otterAnimationState(animal);
    const sprite = getAnimalSpriteFrame('riverOtter', animation.sequence, animation.frame);
    if (!sprite) return false;
    const width = animal.juvenile ? 40 : 50;
    const height = width * sprite.naturalHeight / sprite.naturalWidth;
    ctx.save();
    ctx.translate(0, animation.bob);
    ctx.drawImage(sprite, -width / 2, -height * .89, width, height);
    ctx.restore();
    return true;
}
...
function drawStaticAnimalSprite(animal) {
    const sprite = getAnimalSpriteFrame(animal.species, 'idle', 0);
    if (!sprite) return false;
    const large = ['elephant', 'hippo', 'polarBear', 'gorilla', 'giraffe'].includes(animal.species);
    const small = ['meerkat', 'penguin', 'flamingo', 'pygmyGoat'].includes(animal.species);
    let width = large ? 58 : small ? 42 : 50;
    if (animal.juvenile) width *= .72;
    const height = width * sprite.naturalHeight / sprite.naturalWidth;
    const moving = Math.hypot((animal.targetX ?? Math.floor(animal.px)) + .5 - animal.px, (animal.targetY ?? Math.floor(animal.py)) + .5 - animal.py) > .12;
    const bob = moving ? Math.sin(performance.now() / 155 + animal.animOffset) * .8 : Math.sin(performance.now() / 700 + animal.animOffset) * .25;
    ctx.save();
    ctx.translate(0, bob - height * .02);
    ctx.drawImage(sprite, -width / 2, -height * .87, width, height);
    ctx.restore();
    return true;
}
...
function drawAnimal(a) {
    ...
    if (id !== 'riverOtter' && drawStaticAnimalSprite(a)) {
        // Illustrated sprite asset loaded for this species.
    }
    else if (id === 'zebra') {
